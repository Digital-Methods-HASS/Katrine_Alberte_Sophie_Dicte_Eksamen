---
title: "Danish Kings"
author: "Sophie Løbner, Katrine Pedersen, Dicte Mikkelsen, Alberte Thomsen."
date: "2025-03-05"
...
library(tidyverse)
kings1 <- read.csv("kings.csv")

kings2 <- read_csv("kings.csv",na=c("NULL",""))

kings3 <- read.csv2("kings.csv")

kings4 <- read_csv2("kings.csv")
```
Answer: 
...
You find out if there's numeric or characters in the dataset. If you write "" around the object, you get a direct answer. This can be seen with the second class function. 

3. How many columns does the object have when created with these different functions?
R shows the first 6 rows and all 11 columns
...
Do you remember how to calculate an average on a vector object? So you need to subset your `kings` dataset to the `duration` column. If you subset it as a vector you can calculate average on it with `mean()` base-R function. If you subset it as a tibble, you can calculate average on it with `summarize()` tidyverse function.
